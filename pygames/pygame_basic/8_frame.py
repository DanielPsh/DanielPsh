...
enemy_y_pos = screen_height / 2 - \
    (enemy_height) / 2  # 화면 세로의 절반 크기에 해당하는 곳에 위치
...
# 20 fps : 1초 동안에 20번 동작 -> 1번에 몇 만큼 이동? 5만큼! 5 * 20 = 100
    ...
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / \
        1000  # 경과 시간을 1000으로 나누어서 초(s) 단위로 표시

    # 시간 표시
    timer = game_font.render(
        # render(출력할 글자, True, 글자 색상)
        f"Time : {str(int(total_time - elapsed_time))}", True, (255, 255, 255))
    ...
    pygame.display.update()  # 게임화면을 다시 그리기! (계속 호출되어야 함)
